 ...
 *   getPolynom()      => null
 */
function getPolynom(...mass) {
  return (x) => {
    if (mass.length === 3) {
      return (mass[0] * x ** 2) + mass[1] * x + mass[2];
    }
    if (mass.length === 2) {
      return mass[0] * x + mass[1];
    }
    if (mass.length === 1) {
      return mass[0];
    }
...
function logger(func, logFunc) {
  return (...mass) => {
    logFunc(JSON.stringify(mass).replace(/\[/, `${func.name}(`).replace(/\]$/, ') starts'));
    const str = func(...mass);
    logFunc(JSON.stringify(mass).replace(/\[/, `${func.name}(`).replace(/\]$/, ') ends'));
